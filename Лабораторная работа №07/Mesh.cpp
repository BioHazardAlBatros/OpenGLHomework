				...
				switch (currentField)
				{
					...
					case FACE:
						while(!currentLine.eof())
						{
							...
							std::unordered_map<glm::ivec3, GLuint>::iterator vertIter = vertexToIndexTable.find(fPoints.back());
							...
							vertices.back().pos = vert[fPoints.back()[0] - 1];
							vertices.back().texPos = tex[fPoints.back()[1] - 1];
							vertices.back().normal = norm[fPoints.back()[2] - 1];
						}
						++facesCount;
						break;
				}
